...
```{r chunk5, echo=TRUE, cache=TRUE, dependson=c(-1)}
...
ingresos1$diag1<-NA
foreach(i = 1:nrow(ingresos1)) %dopar%{
  # if (i %in% seq(1,nrow(ingresos1),10000)){
  #   cat(sprintf("\r%s",i))
  # }
  ingresos1[i,]$diag1<-TraduceCodigoMasGeneral(ingresos1[i,]$diag_ppal)
}
stopCluster(cl)
```
El numero total de ingresos urgentes en ese periodo fue de `r nrow(ingresos1)` de ellos, `r nrow(ingresos1 %>% filter(edad<=16))` fueros menores de 17 años y 
